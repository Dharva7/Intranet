...
var f;
   ...
   $(document).on('change','#fac-subjectselect',function () {
        ...
        $.ajax({
          type: "POST",
          url: "http://localhost/test/Intranet/php/get_subject.php",
          data:'sem_id='+t,
    ...
    $(document).on('click', '#addinsfilebtn', function () {
        ...
        // console.log(temp);
        $(temp).uploadFile({
            url: "http://localhost/test/Intranet/php/upload2.php",
            dragDrop: true,
            fileName: "myfile",
            formData: {"name":f},
            returnType: "json",
            ...
            onLoad: function (obj) {
                $.ajax({
                    cache: false,
                    url: "http://localhost/test/Intranet/php/load.php",
                    dataType: "json",
                    success: function (data) {
    ...
    $(document).on('click','#addeditmarks',function () {
        ...
        url: "http://localhost/test/Intranet/php/upload.php",
        dragDrop: true,
        fileName: "myfile",
        formData: {"name":f},
        returnType: "json",
        ...
        deleteCallback: function (data, pd) {
            for (var i = 0; i < data.length; i++) {
                $.post("http://localhost/test/Intranet/php/delete.php", {op: "delete", name: data[i]},
                    function (resp, textStatus, jqXHR) {
                        //Show Message
                        alert("File Deleted");
                    });
        ...
        downloadCallback: function (filename, pd) {
            location.href = "http://localhost/test/Intranet/php/download.php?filename=" + filename;
        }
    });
    ...
    $(document).on('click', '#addeditlabmanual', function () {
        ...
        url: "http://localhost/test/Intranet/php/upload1.php",
        dragDrop: true,
        fileName: "myfile",
        formData: {"name":f},
        returnType: "json",
        ...
        downloadCallback: function (filename, pd) {
         ...
         $.ajax({
          type: "POST",
          url: "http://localhost/test/Intranet/php/temp.php",
          data:'sub_id='+f,
      });
    });
